  ...
  class DasdFmtProgress
    ...
    def tables
      HBox(
        ...
        HSpacing(8),
        # Multiple HStretches to take up more excess width upon resize
        # (each table and each HStretch gets the same share)
        HStretch(),
        HStretch(),
        MinWidth(
          15,
    ...
    def in_progress_items
      [
        Item(Id(:dasda), "/dev/dasda", format_cyl(@cyl, 200)),
        Item(Id(:dasdb), "/dev/dasdb", format_cyl(@cyl, 420)),
        Item(Id(:dasdc), "/dev/dasdc", format_cyl(@cyl, 300)),
        Item(Id(:dasdd), "/dev/dasdd", format_cyl(@cyl, 200)),
        Item(Id(:dasde), "/dev/dasde", format_cyl(@cyl, 200)),
        Item(Id(:dasdf), "/dev/dasdf", format_cyl(@cyl, 200)),
        Item(Id(:dasdg), "/dev/dasdg", format_cyl(@cyl, 200)),
        Item(Id(:dasdh), "/dev/dasdh", format_cyl(@cyl, 200))
      ]
    end
    ...
    def update_cyl_cell(item_id, cyl, total_cyl)
      ...
      UI.ChangeWidget(Id(:in_progress_table), Cell(item_id, 1), format_cyl(cyl, total_cyl))
    end
  end
end
